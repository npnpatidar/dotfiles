...
function runHerdr(args: string[], timeoutMs = 60_000): { ok: boolean; result?: unknown; error?: string } {
	let res = spawnSync("herdr", args, { encoding: "utf8", timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024 });
	...
	if (res.status !== 0) return { ok: false, error: (res.stderr || "").trim() || `herdr exited ${res.status}` };
	try {
...
function listEntries(): any[] {
	...
	try {
		return fs
			...
			.map((f) => {
				try {
					return JSON.parse(fs.readFileSync(path.join(REGISTRY_DIR, f), "utf8"));
				} catch {
					return null;
				}
			})
...
function sanitizeLabel(s: string): string {
	const cleaned = s.replace(/\s+/g, " ").replace(/[^\w\- .+]/g, "-").trim().slice(0, 40);
...
function isSubAgent(eventSystemPrompt: string, appendedSystemPrompt: string | undefined): boolean {
	return Boolean(
		(appendedSystemPrompt && appendedSystemPrompt.includes(MARKER)) || eventSystemPrompt.includes(MARKER),
	);
}
	...
	try {
		...
		for (const f of fs.readdirSync(REGISTRY_DIR)) {
			...
			try {
				...
				const done = e.status === "done" || e.status === "failed" || e.status === "aborted";
				if (done && now - new Date(e.created_at ?? 0).getTime() > 7 * 24 * 3600_000) {
...
function alivePaneSet(): Set<string> | null {
	...
	try {
		const agents = ((r.result as any).agents ?? []) as Array<{ pane_id: string; agent_status: string }>;
		return new Set(
			agents
				.filter((a) => a.agent_status !== "done" && a.agent_status !== "unknown")
				.map((a) => a.pane_id),
		);
	} catch {
		return null;
...
function reclaimDeadSlots(): number {
	...
	for (const e of listEntries()) {
		...
		if (Date.now() - new Date(e.updated_at ?? 0).getTime() < REAP_AGE_MS) continue;
		if (alive.has(e.pane)) continue; // genuinely alive — keep the slot
		writeEntry({ ...e, status: "aborted", report: "Presumed dead: no live agent for 10+ minutes — slot reclaimed." });
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			task: Type.String({ description: "The single, verifiable task for the sub-agent." }),
			name: Type.Optional(Type.String({ description: "Tab label / sub-agent name (default: subagent-<n>)." })),
			cwd: Type.Optional(Type.String({ description: "Working directory for the sub-agent (default: current)." })),
			workspace: Type.Optional(Type.String({ description: "herdr workspace id (default: current workspace)." })),
			wait: Type.Optional(Type.Boolean({ description: "Block until the sub-agent reports (default: true)." })),
			timeoutMinutes: Type.Optional(Type.Number({ description: `Max wait in minutes (default: ${DEFAULT_TIMEOUT_MIN}).` })),
		}),
		async execute(toolCallId, params: any, signal, onUpdate) {
			const update = (text: string) => onUpdate?.({ content: [{ type: "text" as const, text }] });
			...
			if (!workspace) {
				return {
					content: [
						{ type: "text", text: "Cannot determine herdr workspace (HERDR_WORKSPACE_ID is unset). Run pi inside a herdr pane." },
					],
				};
			}
			...
			const name = params.name ? sanitizeLabel(params.name) : `subagent-${listEntries().length + 1}`;

			// --- create tab ---
			update(`Creating herdr tab "${name}"…`);
			const created = runHerdr(["tab", "create", "--workspace", workspace, "--label", name, "--cwd", cwd]);
			if (!created.ok || !created.result) {
				return { content: [{ type: "text", text: `Failed to create tab: ${created.error ?? "unknown error"}` }] };
			}
			const tabId: string = (created.result as any).tab?.tab_id;
			const pane: string = (created.result as any).root_pane?.pane_id;
			if (!tabId || !pane) {
				return { content: [{ type: "text", text: `Tab created but pane/tab ids missing: ${JSON.stringify(created.result)}` }] };
			}
			...
			fs.writeFileSync(
				promptFile,
				buildSubagentPrompt({ pane, tab: tabId, workspace, parentPane, parentTab, cwd, task: params.task }),
			);
			try {
			...
			const labelSlug = (name || "task").toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 20) || "task";
			const paneSlug = pane.toLowerCase().replace(/[^a-z0-9_-]/g, "-");
			const agentName = `sa-${labelSlug}-${paneSlug}`.slice(0, 32);
			let started: { ok: boolean; result?: unknown; error?: string } | undefined;
			for (let attempt = 1; attempt <= 3 && !started?.ok; attempt++) {
				if (attempt > 1) {
					const waitMs = 4_000 * attempt;
					update(`Pane shell not ready yet — retrying agent start in ${waitMs / 1000}s (attempt ${attempt}/3)…`);
					await sleep(waitMs);
				}
				started = runHerdr(
					["agent", "start", agentName, "--kind", "pi", "--pane", pane, "--timeout", String(AGENT_START_TIMEOUT_MS), "--", "pi", "--append-system-prompt", promptFile],
					AGENT_START_TIMEOUT_MS + 15_000,
				);
			}
			if (!started?.ok) {
				writeEntry({ ...entry, status: "failed", report: `Failed to start pi in pane ${pane}: ${started.error}` });
				closeTab(tabId, pane);
				return {
					content: [{ type: "text", text: `Failed to start sub-agent in pane ${pane}: ${started.error}` }],
					details: { pane, tab: tabId, status: "failed", registry: entryPath(pane) },
				};
			}

			// --- kick the sub-agent to begin its task ---
			const kick = runHerdr(
				["agent", "prompt", pane, "Begin your single assigned task NOW. Follow the SUBAGENT ROLE control instructions in your system prompt exactly. When finished or impossible, call subagent_report."],
				30_000,
			);
			if (!kick.ok) {
				...
				if (alive !== null && !alive.has(pane)) {
					writeEntry({ ...(readEntry(pane) ?? entry), status: "failed", report: `Pi did not come up in pane ${pane} (kick failed: ${kick.error}).` });
					closeTab(tabId, pane);
					return {
						content: [{ type: "text", text: `Sub-agent failed to start in pane ${pane} (kick failed: ${kick.error}) — marked failed.` }],
						details: { pane, tab: tabId, status: "failed", registry: entryPath(pane) },
					};
				}
			} else {
			...
			if (params.wait === false) {
				return {
					...
					details: { pane, tab: tabId, status: "working", registry: entryPath(pane) },
				};
			}

			// --- poll for completion ---
			const timeoutMin = Math.min(params.timeoutMinutes ?? DEFAULT_TIMEOUT_MIN, MAX_TIMEOUT_MIN);
			const deadline = Date.now() + timeoutMin * 60_000;
			let lastSeen = "";

			while (Date.now() < deadline) {
				if (signal?.aborted) {
					writeEntry({ ...(readEntry(pane) ?? entry), status: "aborted", report: "Aborted by caller before completion." });
					closeTab(tabId, pane);
					return { content: [{ type: "text", text: `Sub-agent ${pane} aborted by caller.` }] };
				}
				await sleep(POLL_INTERVAL_MS);
				const e = readEntry(pane);
				if (e) {
					if (e.status === "done" || e.status === "failed" || e.status === "aborted") {
						...
						return {
							...
							details: { pane, tab: tabId, status, report, registry: entryPath(pane) },
						};
					}
					if (e.updated_at && e.updated_at !== lastSeen) {
						lastSeen = e.updated_at;
						update(`Sub-agent ${pane} (${name}): status ${e.status} — still working…`);
					}
				}
			}
			...
			// --- timeout ---
			const e = readEntry(pane);
			writeEntry({ ...(e ?? entry), status: "aborted", report: `Timed out after ${timeoutMin} minutes without a report.` });
			closeTab(tabId, pane);
			return {
				content: [
					{
				...
				details: { pane, tab: tabId, status: "aborted", registry: entryPath(pane) },
			};
		},
	});
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			pane: Type.String({ description: "Pane id of the sub-agent to abort (e.g. w9:pK), from subagents_list." }),
			reason: Type.Optional(Type.String({ description: "Optional reason recorded in the entry's report field." })),
		}),
		async execute(toolCallId, params: any) {
			const entry = readEntry(params.pane);
			if (!entry) {
				return { content: [{ type: "text", text: `No registry entry for pane ${params.pane} — nothing to abort.` }] };
			}
			if (entry.status !== "working") {
				return { content: [{ type: "text", text: `Sub-agent ${params.pane} is already ${entry.status} — nothing to abort.` }] };
			}
			const reason = params.reason ? ` Aborted by main agent: ${params.reason}` : " Aborted by main agent.";
			writeEntry({ ...entry, status: "aborted", report: `${truncate(entry.report ?? "", 500)}${reason}` });
			...
			return {
				...
				details: { pane: params.pane, status: "aborted", registry: entryPath(params.pane) },
			};
		},
	});
	...
	pi.registerTool({
		...
		async execute() {
			...
			if (entries.length === 0) {
				return { content: [{ type: "text", text: "No sub-agents in the registry yet." }] };
			}
			...
			const lines = entries
				...
				.map((e, i) => {
					...
					const dead = alive !== null && e.status === "working" && quietMs > 10 * 60_000 && !alive.has(e.pane);
					const ageMin = Math.max(0, Math.round((Date.now() - new Date(e.created_at ?? 0).getTime()) / 60_000));
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			report: Type.String({ description: "Final report: what you did, evidence, results, issues." }),
			failed: Type.Optional(Type.Boolean({ description: "Set true when the task could not be completed." })),
		}),
		async execute(toolCallId, params: any) {
			const pane = currentPaneId();
			if (!pane) {
				return { content: [{ type: "text", text: "Cannot self-identify: HERDR_PANE_ID is unset (not running inside a herdr pane)." }] };
			}
			...
			if (entry.status === "done" || entry.status === "failed") {
				return {
					content: [{ type: "text", text: `Report already recorded (status ${entry.status}). Refusing to overwrite the final report.` }],
				};
			}
	...
	pi.registerTool({
		...
		async execute(toolCallId, params: any) {
			const pane = currentPaneId();
			if (!pane) return { content: [{ type: "text", text: "Cannot self-identify: HERDR_PANE_ID is unset." }] };
			const entry = readEntry(pane);
			if (!entry) return { content: [{ type: "text", text: `No registry entry for pane ${pane}.` }] };
	...
	function reconcileRole(appended: string | undefined, systemPrompt: string): void {
		const isSub = isSubAgent(systemPrompt, appended);
		const active = pi.getActiveTools();
		if (isSub) {
			const remove = [SPAWN_TOOL, ...MAIN_TOOLS];
			if (remove.some((t) => active.includes(t))) pi.setActiveTools(active.filter((t) => !remove.includes(t)));
		} else {
			const hasSubTools = SUB_TOOLS.some((t) => active.includes(t));
			if (hasSubTools) pi.setActiveTools(active.filter((t) => !SUB_TOOLS.includes(t)));
		}
	}
